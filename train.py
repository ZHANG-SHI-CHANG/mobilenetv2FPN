...
from Mobilenetv2 import Mobilenetv2,Detection_or_Classifier
...
class Train:
    ...
    def __init__(self, sess):
        ...
        if Detection_or_Classifier=='classifier':
            ...
            self.train_data = DataLoader(root=self.dataset_root+'SmallNORB\\trainImages',classes=len(self.labels),batch=batch_size)
            self.test_data = DataLoader(root=self.dataset_root+'SmallNORB\\testImages',classes=len(self.labels),batch=batch_size)
            
            print("Building the model...")
            self.model = Mobilenetv2(num_classes=len(self.labels),
                                  num_anchors=5,
